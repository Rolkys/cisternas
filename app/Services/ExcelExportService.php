...
class ExcelExportService
{
    ...
    public function export($cisternas)
    {
        ...
        foreach($cisternas as $cisterna){
            ...
            $sheet->setCellValue('J' . $row, $cisterna->HoraSalida?->format('d/m/Y H:i'));
            $sheet->setCellValue('K' . $row, $cisterna->FechaEntradaMG?->format('d/m/Y H:i'));
            $sheet->setCellValue('L' . $row, $cisterna->FechaConsumoMG?->format('d/m/Y'));
            $sheet->setCellValue('M' . $row, $cisterna->HoraEstimadaConsumoL1?->format('H:i'));
            $sheet->setCellValue('N' . $row, $cisterna->HoraEstimadaConsumoL2?->format('H:i'));
            $sheet->setCellValue('O' . $row, $cisterna->HoraRealConsumoL1?->format('H:i'));
            $sheet->setCellValue('P' . $row, $cisterna->HoraRealConsumoL2?->format('H:i'));
