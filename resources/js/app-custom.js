...
============================================================ */
    ...
    ────────────────────────────────────────────── */
function abrieModal(id, hecL1, hrcL1, hecL2, hrcL2, obs){
    ...
    l1.disable = false;
    l2.disable = false;
    if(hrcL1)       {l2.disable = true;}
    else if(hrcL2)  {l1.disable = true;}

    const modal= new boostrap.Modal(document.getElementById('modalConsumo'));
    modal.show();
}

/* Mutex en el tiempo real dentro el modal */
document.addEventListener('DOMContentLoaded', function(){
    ...
    if(l1 && l2){
        l1.addEventListener('input', function(){
            if(this.value)  {l2.value = ''; l2.disable = true;}
            else            {l2.disable = false;}
        });
        l2.addEventListener('input', function(){
            if(this.value)  {l1.value = ''; l1.disable = true;}
            else            {l1.disable = false;}
        });
    }
    ...
    window.generarPassword=function(){
        ...
        if(!parte) { alert('Introduce primero el email.'); return;}
        ...
        if (btnEl) btnEl.removeAttribute('disable');
    };
    ...
    document.querySelectorAll('.hec-l1').forEach(function (l1c) {
        ...
        l1c.addEventListener('input', function () {
            if (this.value) { l2c.value = ''; l2c.disabled = true; }
            else            { l2c.disabled = false; }
        });
        l2c.addEventListener('input', function () {
            if (this.value) { l1c.value = ''; l1c.disabled = true; }
            else            { l1c.disabled = false; }
        });

        if (l1c.value)      { l2c.disabled = true; }
        else if (l2c.value) { l1c.disabled = true; }
    });

    /*──────────────────────────────────────────────────────
        Seleccionador / deseleccionador todos (bulk_confirm)
    ──────────────────────────────────────────────────────*/

    window.toggleTodos = function (estado){
        documen.querySelectorAll('.check-fila').forEach(cb => cb.checked = estado);
    };
    ...
    function setupMutex(idA, idB){
        ...
        if(a.value)         {b.disable=true;}
        else if (b.value)   {a.disable=true;}

        a.addEventListener('input', function(){
            if(this.value)  {b.value=''; b.disable = true;}
            else            {b.disable = false;}
        });

        b.addEventListener('input', function(){
            if(this.value)  {a.value=''; a.disable = true;}
            else            {a.disable = false;}
        });
    }
